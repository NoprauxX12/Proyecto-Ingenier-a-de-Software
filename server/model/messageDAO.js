...
exports.insterMessage= async (content, autor, estimateid, time, cb)=>{
     connection.query('INSERT INTO messages (content, autor, estimateid, time, visto) VALUES (?, ?, ?, ?,?)', [content, autor, estimateid, time, 0], (err, result) => {
        if (err) {
...
exports.notifications= (json, cb)=>{
  ...
  connection.query(sql, [idUser, name], async (err, res)=>{
    if(err){
      ...
      let sql =user==="2"? "select count(*) cuenta from estimate where state_stateId=1 and idClient=?":
      "select count(*) cuenta from estimate where state_stateId=1 and idFreelancer=?";
    ...
    console.log("notificaciones"+cuenta)
